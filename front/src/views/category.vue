<template>
  <div class="content">
        ...
        <div class="category-content">
          <ul>
            <div class="first-category">
              <span class="first-category-title">一级分类：</span>
            </div>
            <li
            ...
            >
              <span
                ...
                :class="item.id == categoryId ? 'activate' : ''"
                >{{ item.name }}</span
              >
            </li>
          </ul>
    ...
    <div class="book-list-box">
      <div class="sorted-header"></div>
      <div class="book-content" v-if="data.books.length">
            ...
            >
              ...
              <div class="item-info">
                ...
                <div class="f_left new-message-box" v-if="item.messageInfo">
                  <a class="new-message" :href="'sbook/' + item.messageInfo.id">
...
let categoryId = ref<number>(proxy.$route.query.id);
...
onBeforeMount(() => {
  axios.get(proxy.$API.API_CATEGORY).then((res: any) => {
    data.category = res.results;
  });
  if (categoryId.value) {
    axios
      .get(proxy.$API.API_CATEGORYBOOK + `${categoryId.value}/`)
      .then((res: any) => {
...
watch(
  () => categoryId.value,
  (value, oldValue) => {
    axios.get(proxy.$API.API_CATEGORYBOOK + `${value}/`).then((res: any) => {
      data.books = res.results;
      totalCount.value = res.count;
      console.log(res);
    });
